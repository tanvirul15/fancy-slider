...
const selectItem = (event, img) => {
    ...
    if (item === -1) {
        ...
        sliders = sliders.filter((val, ind) => {
            return item != ind;
        })
    }
}
...
searchInput.addEventListener('keypress', e => { if (e.keyCode === 13) searchImage() });

durationInput.addEventListener('keypress', e => {
    if (e.keyCode === 13) createSlider();
});


//Search Image function: when search button clicked
